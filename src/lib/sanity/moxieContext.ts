...
// Fallback configuration matching Next.js 14 conventions
const client = createClient({
  projectId: process.env.NEXT_PUBLIC_SANITY_PROJECT_ID || '',
  dataset: process.env.NEXT_PUBLIC_SANITY_DATASET || 'production',
  apiVersion: '2024-01-01',
  useCdn: false, // Set false so Moxie always gets up-to-the-minute changes
})
...
export async function getUbuntuCMSContext(): Promise<string> {
  ...
  try {
    ...
    // Format data cleanly to save LLM tokens while preserving high contextual accuracy
